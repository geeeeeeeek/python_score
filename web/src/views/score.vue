      ...
      >
        ...
        <div style="padding-right: 16px; max-height: 480px; overflow-x: hidden; overflow-y: auto">
          <a-form ref="myform" :label-col="{ style: { width: '80px' } }" :model="modal.form" :rules="modal.rules">
            <a-row :gutter="24">
              <a-col span="12">
                <a-form-item label="学生" name="thing">
                  <a-select
                    placeholder="请选择"
                    allowClear
                    :options="modal.thingData"
                    :field-names="{ label: 'title', value: 'id' }"
                    v-model:value="modal.form.thing"
                  />
                </a-form-item>
              </a-col>
              <a-col span="12">
                <a-form-item label="课程" name="course">
                  <a-select
                    placeholder="请选择"
                    allowClear
                    :options="modal.courseData"
                    :field-names="{ label: 'title', value: 'id' }"
                    v-model:value="modal.form.course"
                  />
                </a-form-item>
              </a-col>
              <a-col span="12">
                <a-form-item label="分数" name="score">
                  <a-input placeholder="请输入" v-model:value="modal.form.score" />
                </a-form-item>
              </a-col>
            </a-row>
          </a-form>
...
<script setup lang="ts">
  ...
  import { listApi as listCourseApi } from '/@/api/course';
  ...
  const modal = reactive({
    ...
    form: {
      id: undefined,
      title: undefined,
      thing: undefined, // 对应学生
      course: undefined, // 对应课程
      score: undefined,
    },
    rules: {
      title: [{ required: true, message: '请输入', trigger: 'change' }],
      thing: [{ required: true, message: '请选择', trigger: 'change' }],
  ...
  const handleOk = () => {
    myform.value
      ...
      .then(() => {
        ...
        if (modal.form.thing) {
          formData.append('thing', modal.form.thing);
        }
        if (modal.form.course) {
          formData.append('course', modal.form.course);
        }
        if (modal.editFlag) {
